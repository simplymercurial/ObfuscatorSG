...
; Predefine variables
; ========================================
Global Const $g_sTitle = 'UpdateIt'
...
GUICtrlCreateLabel($g_sTitle & ' v' & $s_Version & ' - The AutoIt3 Update Utility written by Rob Saunders.' & @LF, 5, 5, 290, 75)
...
While 1
	...
	If Not $i_DatFileLoaded And InetGetInfo($g_hInetGet, 2) Then
		If InetGetInfo($g_hInetGet, 3) = False And $b_Download_UpdateDat Then
			$i_Res = MsgBox(5 + 16 + 8192, 'Error', 'Error connecting to server.' & @LF)
			If $i_Res = 4 Then
				$g_hInetGet = InetGet($s_DatFile, $g_sDatFile_Local, 1, 1)
			Else
				Exit
